...
int main(void) {
    ...
    // Init UART1 for NRF52840 communication (future use)
        ...
        if (gamepad.updated) {
            ...
            if (now - last_print > 200) {
                ...
                printf("BTN:%04X  A:%d B:%d X:%d Y:%d  LB:%d RB:%d  "
                       "LT:%3d RT:%3d  LX:%6d LY:%6d  RX:%6d RY:%6d\n",
                    gamepad.buttons,
            ...
            // Send binary packet to NRF52840
            uart_send_to_nrf();
        }
    }
