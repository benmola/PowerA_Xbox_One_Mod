...
// Connect a USB-to-TTL adapter: adapter TX→board RX, adapter RX→board TX,
// GND→GND. Open the COM port at 115200 baud to see debug output.
...
#define GIP_CMD_IDENTIFY                                                       \
  0x04                     // host requests / device responds with capabilities
#define GIP_CMD_POWER 0x05 // host sends power mode (on/off/sleep/reset)
#define GIP_CMD_AUTHENTICATE                                                   \
  0x06 // crypto auth handshake (or skip via AUTH COMPLETE)
...
typedef struct __attribute__((packed)) {
  ...
  uint8_t options;  // flags (0x00 for normal input)
  uint8_t sequence; // incrementing counter (1-255, 0 = no sequence)
...
tud_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage,
  ...
  if (stage != CONTROL_STAGE_SETUP)
    return true; // ACK data/status stages
  ...
  if (wait_ms > 0) {
    ...
    while (tud_vendor_write_available() < len) {
      if (millis() - t0 >= wait_ms) {
        Serial.printf("[GIP] FIFO write timeout (%lums), dropped %lu bytes  "
                      "fifo_free=%lu\r\n",
...
void gip_send_announce() {
  static const uint8_t announce[32] = {
      ...
      GIP_OPT_INTERNAL,
      0x00,
      0x1C, // header: cmd=0x02, opts=0x20, seq=0, len=28
      // address[6] — fake MAC
      0x00,
      0x00,
      0x00,
...
void gip_send_identify(uint8_t host_seq) {
  uint8_t pkt0[64] = {
      0x04, 0xF0, 0x00, 0x3A, 0xCA, 0x01, 0x10, 0x00, 0x01, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xCA, 0x00,
      0x8B, 0x00, 0x16, 0x00, 0x1F, 0x00, 0x20, 0x00, 0x27, 0x00, 0x2D,
      0x00, 0x4A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x02, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x06,
      0x01, 0x02, 0x03, 0x04, 0x06, 0x07, 0x05, 0x01, 0x04,
  };
  pkt0[2] = host_seq;
  if (!gip_safe_write(pkt0, sizeof(pkt0), 500)) {
    Serial.println("[GIP] IDENTIFY chunk 0 dropped");
    return;
  }
  delay(40); // Required for timing on PC (xboxgip.sys drops fast packets)

  uint8_t pkt1[64] = {
      0x04, 0xA0, 0x00, 0xBA, 0x00, 0x3A, 0x05, 0x06, 0x0A, 0x01, 0x1A,
      0x00, 0x57, 0x69, 0x6E, 0x64, 0x6F, 0x77, 0x73, 0x2E, 0x58, 0x62,
      0x6F, 0x78, 0x2E, 0x49, 0x6E, 0x70, 0x75, 0x74, 0x2E, 0x47, 0x61,
      0x6D, 0x65, 0x70, 0x61, 0x64, 0x04, 0x56, 0xFF, 0x76, 0x97, 0xFD,
      0x9B, 0x81, 0x45, 0xAD, 0x45, 0xB6, 0x45, 0xBB, 0xA5, 0x26, 0xD6,
      0x2C, 0x40, 0x2E, 0x08, 0xDF, 0x07, 0xE1, 0x45, 0xA5,
  };
  pkt1[2] = host_seq;
  if (!gip_safe_write(pkt1, sizeof(pkt1), 500)) {
    Serial.println("[GIP] IDENTIFY chunk 1 dropped");
    return;
  }
  delay(40);

  uint8_t pkt2[64] = {
      0x04, 0xA0, 0x00, 0xBA, 0x00, 0x74, 0xAB, 0xA3, 0x12, 0x7A, 0xF1,
      0x97, 0xB5, 0xE7, 0x1F, 0xF3, 0xB8, 0x86, 0x73, 0xE9, 0x40, 0xA9,
      0xF8, 0x2F, 0x21, 0x26, 0x3A, 0xCF, 0xB7, 0xFE, 0xD2, 0xDD, 0xEC,
      0x87, 0xD3, 0x94, 0x42, 0xBD, 0x96, 0x1A, 0x71, 0x2E, 0x3D, 0xC7,
      0x7D, 0x02, 0x17, 0x00, 0x20, 0x20, 0x00, 0x01, 0x00, 0x10, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  };
  pkt2[2] = host_seq;
  if (!gip_safe_write(pkt2, sizeof(pkt2), 500)) {
    Serial.println("[GIP] IDENTIFY chunk 2 dropped");
    return;
  }
  delay(40);

  uint8_t pkt3[34] = {
      0x04, 0xB0, 0x00, 0x1C, 0xAE, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x17,
      0x00, 0x09, 0x3C, 0x00, 0x01, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  };
  pkt3[2] = host_seq;
  if (!gip_safe_write(pkt3, sizeof(pkt3), 500)) {
    Serial.println("[GIP] IDENTIFY chunk 3 dropped");
    return;
  }
  delay(40);
  ...
  pkt4[2] = host_seq;
  if (!gip_safe_write(pkt4, sizeof(pkt4), 500)) {
    Serial.println("[GIP] IDENTIFY chunk 4 dropped");
    return;
  }
  delay(40);
...
//    Payload: [unknown, acked_cmd, acked_options, acked_len[2], pad[2],
//    remaining[2]] Total payload = 9 bytes
//
...
void gip_handle_rx() {
  while (tud_vendor_available()) {
    ...
    if (host_cmd == GIP_CMD_IDENTIFY) {
      ...
      if (mode == 0x00 || mode == 0x01 || mode == 0x04 || mode == 0xFF) {
      ...
      Serial.println(
          "[GIP] AUTHENTICATE (0x06) — sending AUTH COMPLETE to skip");
...
void setup() {
  ...
  TinyUSBDevice
      .clearConfiguration(); // strip ALL (CDC + vendor + VID/PID/strings!)
  ...
  //
  // Note: we read from the cached config descriptor
  // (tud_descriptor_configuration_cb) NOT from getInterfaceDescriptor() —
  // calling that again would allocate duplicate interface/endpoint numbers as a
  // side effect.
  {
    extern uint8_t const *tud_descriptor_configuration_cb(uint8_t index);
    const uint8_t *cfg = tud_descriptor_configuration_cb(0);
    // Config descriptor header is 9 bytes; our interface descriptor starts at
    // offset 9
    ...
    if (desc_len >= 23) {
      Serial.printf(
          "  Itf#=%d  SubClass=0x%02X Proto=0x%02X  EP_OUT=0x%02X(attr=%02X "
          "int=%d)  EP_IN=0x%02X(attr=%02X int=%d)\r\n",
          desc_buf[2], desc_buf[6], desc_buf[7], desc_buf[11], desc_buf[12],
          desc_buf[15], desc_buf[18], desc_buf[19], desc_buf[22]);
      bool ok =
          (desc_buf[2] == 0x00 && desc_buf[6] == 0x47 && desc_buf[7] == 0xD0 &&
           desc_buf[12] == 0x03 && desc_buf[19] == 0x03);
      Serial.printf("  GIP patch: %s\r\n",
                    ok ? "OK (Itf#0, Interrupt endpoints)"
                       : (desc_buf[2] != 0x00
                              ? "WRONG INTERFACE NUMBER (must be 0!)"
  ...
  while (!USBDevice.mounted())
    delay(1);
  Serial.println("USB mounted. Sending GIP ANNOUNCE, then waiting for host "
                 "IDENTIFY request (0x04)...");
...
void loop() {
  ...
  if (tud_vendor_mounted()) {
    if (!gip_ever_mounted) {
      ...
      Serial.println(
          "[GIP] sending ANNOUNCE only — waiting for host IDENTIFY request");
    ...
    if (gip_ever_mounted) {
      ...
      Serial.printf("[GIP] vendor unmounted — state reset (total OUT pkts this "
                    "session: %lu)\r\n",
                    gip_out_pkt_count);
      gip_out_pkt_count = 0;
    }
  ...
  if (NRF_RADIO->EVENTS_END) {
    ...
    if (NRF_RADIO->CRCSTATUS == 1) {
      ...
      if (now_ms - last_print_ms >= 500) {
        ...
        Serial.printf("BTN:%04X b1:%02X b2:%02X LX:%6d LY:%6d RX:%6d RY:%6d "
                      "LT:%3d RT:%3d\r\n",
                      c_buttons, map_gip_btn1(c_buttons),
                      map_gip_btn2(c_buttons), c_lx, c_ly, c_rx, c_ry, c_lt,
                      c_rt);
      }
    } else {
      crc_errors++;
  ...
  if (tud_vendor_mounted() && gip_state == GIP_STATE_READY) {
    ...
    if (now_ms - last_gip_send_ms >= GIP_REPORT_INTERVAL_MS) {
      ...
      bool changed =
          (c_buttons != last_c_buttons || c_lx != last_c_lx ||
           c_ly != last_c_ly || c_rx != last_c_rx || c_ry != last_c_ry ||
...
#define GIP_FALLBACK_MS 5000
    ...
    if (now_wait - last_wait_log_ms >= 1000) {
      ...
      const char *st = (gip_state == GIP_STATE_ANNOUNCED)    ? "ANNOUNCED"
                       : (gip_state == GIP_STATE_IDENTIFIED) ? "IDENTIFIED"
                                                             : "IDLE";
      Serial.printf("[WAIT] %lums  state=%s  fifo_free=%lu  rx_avail=%d  "
                    "out_pkts=%lu\r\n",
                    now_wait - gip_mounted_ms, st, tud_vendor_write_available(),
                    (int)tud_vendor_available(), gip_out_pkt_count);
    }
  ...
  if (now_ms - last_status_ms >= 5000) {
    ...
    Serial.printf("[STATUS] up=%lus pkt=%lu crc_e=%lu radio_st=%lu\r\n",
    ...
    const char *state_str = (gip_state == GIP_STATE_IDLE)         ? "IDLE"
                            ...
                            : (gip_state == GIP_STATE_IDENTIFIED) ? "IDENTIFIED"
                                                                  : "READY";
    Serial.printf("  vendor=%d announced=%d state=%s seq=%d\r\n",
                  (int)tud_vendor_mounted(), (int)gip_announced, state_str,
                  gip_seq);
    Serial.printf("  fifo_free=%lu  rx_avail=%d  OUT_pkts=%lu\r\n",
